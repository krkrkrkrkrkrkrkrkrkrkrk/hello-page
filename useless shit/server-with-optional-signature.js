...
const HEARTBEAT_TIMEOUT = parseInt(process.env.HEARTBEAT_TIMEOUT) || 35000;
const REQUIRE_SIGNATURE_VERIFICATION = process.env.REQUIRE_SIGNATURE_VERIFICATION === 'true';
...
const SERVER_SECRET_KEY = process.env.SERVER_SECRET_KEY || 'server_secret_key_change_this_in_production';
...
console.log(`- Signature Verification: ${REQUIRE_SIGNATURE_VERIFICATION ? 'Required' : 'Optional'}`);
...
wss.on('connection', async (ws, req) => {
  ...
  clients.set(ws, {
    ...
    requireSignature: REQUIRE_SIGNATURE_VERIFICATION // Use global setting by default
  });
  
  console.log(`Client ${id} connected with client ID ${clientId}`);
  console.log(`Signature verification for this client: ${REQUIRE_SIGNATURE_VERIFICATION ? 'Required' : 'Optional'}`);
  ...
  ws.on('message', async (message) => {
    try {
      ...
      // Check if this client requires signature verification
      const client = clients.get(ws);
      const requireSignature = client ? client.requireSignature : REQUIRE_SIGNATURE_VERIFICATION;
      
      // Skip signature verification if it's not required or for initial connection
      if (requireSignature && data.type !== 'initial_connection' && !data.skipSignatureCheck) {
        // Verify signature
        if (!signature) {
      ...
      // Handle configuration message to toggle signature verification for this client
      if (data.type === 'config') {
        if (data.requireSignature !== undefined) {
          const client = clients.get(ws);
          if (client) {
            const oldSetting = client.requireSignature;
            client.requireSignature = Boolean(data.requireSignature);
            
            console.log(`Client ${id} changed signature verification from ${oldSetting} to ${client.requireSignature}`);
            
            const configResponse = {
              type: 'config_response',
              success: true,
              message: `Signature verification ${client.requireSignature ? 'enabled' : 'disabled'} for this connection`,
              requireSignature: client.requireSignature
            };
            
            ws.send(JSON.stringify({
              ...configResponse,
              signature: client.requireSignature ? generateSignature(configResponse, SERVER_SECRET_KEY) : undefined
            }));
          }
        }
        return;
      ...
      if (data.type === 'validate') {
        ...
        if (!validationServiceId || !validationHwid || !validationKey) {
          ...
          ws.send(JSON.stringify({
            ...errorResponse,
            signature: requireSignature ? generateSignature(errorResponse, SERVER_SECRET_KEY) : undefined
          }));
          return;
        }
        ...
        if (validationResult.isValid) {
          ...
          const successResponse = {
            ...
            challengeToken: requireSignature ? client.challengeToken : undefined
          };
          
          ws.send(JSON.stringify({
            ...successResponse,
            signature: requireSignature ? generateSignature(successResponse, SERVER_SECRET_KEY) : undefined
          }));
        } else {
          ...
          ws.send(JSON.stringify({
            ...failureResponse,
            signature: requireSignature ? generateSignature(failureResponse, SERVER_SECRET_KEY) : undefined
          }));
        }
      }
      // Handle heartbeat message
        ...
        if (client) {
          ...
          // Check if challenge response is included when signature verification is required
          if (requireSignature && data.challengeResponse) {
          ...
          ws.send(JSON.stringify({
            ...heartbeatResponse,
            signature: requireSignature ? generateSignature(heartbeatResponse, SERVER_SECRET_KEY) : undefined
          }));
        }
      }
      // Handle challenge-response
      else if (data.type === 'challenge_response' && requireSignature) {
        const client = clients.get(ws);
        if (!client) return;
      ...
      ws.send(JSON.stringify({
        ...errorResponse,
        signature: REQUIRE_SIGNATURE_VERIFICATION ? generateSignature(errorResponse, SERVER_SECRET_KEY) : undefined
      }));
    }
  });
  ...
  const welcomeMessage = {
    ...
    clientId: id,
    requireSignature: REQUIRE_SIGNATURE_VERIFICATION,
    challengeToken: REQUIRE_SIGNATURE_VERIFICATION ? client.challengeToken : undefined
  };
  
  ws.send(JSON.stringify({
    ...welcomeMessage,
    signature: REQUIRE_SIGNATURE_VERIFICATION ? generateSignature(welcomeMessage, SERVER_SECRET_KEY) : undefined
  }));
});
...
app.get('/', (req, res) => {
  const signatureStatus = REQUIRE_SIGNATURE_VERIFICATION ? 'Required' : 'Optional';
  res.send(`WebSocket Server with Heartbeat is running<br>Signature Verification: ${signatureStatus}`);
});
